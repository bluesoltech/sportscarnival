...
const MyComponent = () => {
  ...
  return (
    <div className="relative w-full mt-[100px]">
      ...
      <div className="relative z-10 p-10 overflow-hidden">
        {isMobile ? (
          <Slider {...settings}>
            {data.map((item, index) => (
              <div
                key={index}
                className="relative overflow-hidden w-[300px] h-[300px] mx-4 my-4  rounded-lg flex items-center justify-center p-10"
              >
                <img
                  src={item.imgSrc}
                  ...
                  alt={item.alt}
                />
                <div className="absolute inset-0 opacity-0 transition-opacity duration-300 hover:opacity-100 bg-gray-800 bg-opacity-75 flex items-center justify-center text-white m-10 rounded">
                  {item.content}
                </div>
              </div>
            ))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 text-sm justify-between mt-11 mx-auto overflow-hidden">
            {data.map((item, index) => (
              <div
                key={index}
                className="relative overflow-hidden w-[250px] h-[280px] item-center border-4 border-black rounded-lg mx-auto my-4"
              >
